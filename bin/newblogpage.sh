...
addtomenu () {
    ...
    shift
    am_title="$*"
    new_line="<li><p href='index.html?page=$(basename $am_page_name .html)&root=$(dirname $am_page_name)'>$am_title</p></li>"
...
$(grep -Fhs "page=index" ${content_file}.bak | sort)
            $new_line
$(grep -Fhs "<li><p href=" ${content_file}.bak | sort -r | grep -v "page=index")
        </ul>
    </body>
</html>
ADDTOMENU
